...
def build_seen_from_db(session) -> dict:
    seen = {}
    cols = [
        Comments.id, Comments.title, Comments.body, Comments.created_at,
        Comments.rate, Comments.recommendation_status, Comments.is_buyer,
        Comments.product_id, Comments.advantages, Comments.disadvantages,
        Comments.likes, Comments.dislikes, Comments.seller_title,
        Comments.seller_code, Comments.true_to_size_rate,
    ]
    ...
    for row in session.query(*cols).yield_per(5000):
        ...
        seen[record["id"]] = (null_count, date_key, "committed", None)

    return seen


def load_comments(df: pd.DataFrame, session, seen: dict):
    t0 = time.time()
    ...
    for record in records:
        ...
        null_count = _null_count(record)
        date_key = parse_persian_date(record.get("created_at"))
        new_key = (null_count, -date_key)

        if rid not in seen:
            idx = len(to_insert)
            to_insert.append(record)
            seen[rid] = (null_count, date_key, "pending", idx)
            continue

        existing_null, existing_date, status, ref = seen[rid]
        existing_key = (existing_null, -existing_date)

        if new_key < existing_key:
            if status == "pending":
                to_insert[ref] = record
                seen[rid] = (null_count, date_key, "pending", ref)
            else:
                to_update[rid] = record
                seen[rid] = (null_count, date_key, "committed", None)

    t_loop = time.time()

    if to_insert:
        session.bulk_insert_mappings(Comments, to_insert, render_nulls=True)
    t_insert = time.time()

    if to_update:
        session.bulk_update_mappings(Comments, list(to_update.values()))
    t_update = time.time()
