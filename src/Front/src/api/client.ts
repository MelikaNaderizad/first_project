...
export function normalizeSeller(s: any): SellerItem {
  const satisfaction = Number(s.customer_satisfaction_score ?? (s.satisfaction_score ?? 0));
...
export const apiClient = {
  async getOverview(): Promise<OverviewData> {
    ...
    return {
      ...
      sentimentTimeline: Array.isArray(json.sentimentTimeline) ? json.sentimentTimeline : [],
  ...
  async getComments(params?: {
    ...
    const pageSize = params?.page_size || params?.limit || 21;
    ...
    // In case the backend returns a flat array of comments
    if (Array.isArray(json)) {
      ...
      return {
        metrics: {
          ...
          positive_comments,
          negative_comments,
          positive_rate: total_comments > 0 ? Number(((positive_comments / total_comments) * 100).toFixed(1)) : 0,
          negative_rate: total_comments > 0 ? Number(((negative_comments / total_comments) * 100).toFixed(1)) : 0,
          average_rating: avgRating,
          avg_comments_per_product: total_comments,
          change_rate: '۰٪',
        },
        ratingDistribution: starCounts,
        comments: rawList.slice(0, pageSize),
        totalCount: total_comments,
        page,
        limit: pageSize,
        total_pages: Math.max(1, Math.ceil(total_comments / pageSize)),
      };
    }

    const rawComments = Array.isArray(json.comments) ? json.comments.map(normalizeComment) : [];
    const totalCount = Number(json.totalCount ?? json.total_comments ?? rawComments.length);
    const resolvedLimit = Number(json.limit ?? json.page_size ?? pageSize);
    const totalPages = Number(json.total_pages ?? Math.max(1, Math.ceil(totalCount / (resolvedLimit || 1))));
    ...
    return {
      metrics: {
        ...
        avg_comments_per_product: Number(json.metrics?.avg_comments_per_product ?? 0),
        change_rate: String(json.metrics?.change_rate ?? ''),
      },
      ratingDistribution: Array.isArray(json.ratingDistribution) ? json.ratingDistribution : [],
  ...
  async getProducts(params?: {
    ...
    if (Array.isArray(json)) {
      ...
      return {
        ...
        categoryBreakdown: [],
        products: rawList.slice(0, pageSize),
        page,
        page_size: pageSize,
        totalCount: total_products,
        total_pages: Math.max(1, Math.ceil(total_products / pageSize)),
      };
    }

    const rawProducts = Array.isArray(json.products) ? json.products.map(normalizeProduct) : [];
    const totalCount = Number(json.totalCount ?? json.total_products ?? rawProducts.length);
    const resolvedPageSize = Number(json.page_size ?? pageSize);
    const totalPages = Number(json.total_pages ?? Math.max(1, Math.ceil(totalCount / (resolvedPageSize || 1))));
    ...
    return {
      ...
      categoryBreakdown: Array.isArray(json.categoryBreakdown) ? json.categoryBreakdown : [],
  ...
  async getSellers(params?: {
    ...
    if (Array.isArray(json)) {
      ...
      return {
        ...
        sellers: rawList.slice(0, pageSize),
        page,
        page_size: pageSize,
        totalCount: total_sellers,
        totalPages: Math.max(1, Math.ceil(total_sellers / pageSize)),
      };
    }

    const rawSellers = Array.isArray(json.sellers) ? json.sellers.map(normalizeSeller) : [];
    const totalCount = Number(json.totalCount ?? json.total_sellers ?? rawSellers.length);
    const resolvedPageSize = Number(json.page_size ?? pageSize);
    // Note: Backend uses totalPages for Sellers, while Comments & Products use total_pages
    const totalPages = Number(json.totalPages ?? json.total_pages ?? Math.max(1, Math.ceil(totalCount / (resolvedPageSize || 1))));
    ...
    return {
      metrics: {
        ...
        avg_satisfaction_score: Number(json.metrics?.avg_satisfaction_score ?? 0),
      ...
      performanceComparison: Array.isArray(json.performanceComparison) ? json.performanceComparison : [],
      sellers: rawSellers,
      page: Number(json.page ?? page),
      page_size: resolvedPageSize,
      totalCount,
